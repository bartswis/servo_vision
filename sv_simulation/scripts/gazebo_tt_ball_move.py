...
if __name__ == '__main__':
    ...
    for i in range(0,4*sp,1):
        move(0.01, 0.0, -math.sin(i*math.pi/sp)/sp, math.cos(i*math.pi/sp)/sp)
    for i in range(4*sp,0,-1):
        move(0.01, 0.0, math.sin(i*math.pi/sp)/sp, -math.cos(i*math.pi/sp)/sp)
    '''
    '''
    for i in range(0,2*sp,1):
        move(0.01, math.sin(i*math.pi/sp)/sp, math.cos(i*math.pi/sp)/sp, 0.0)
    for i in range(2*sp,0,-1):
        move(0.01, -math.sin(i*math.pi/sp)/sp, -math.cos(i*math.pi/sp)/sp, 0.0)
    '''
    '''
    for i in range(0,2*sp,1):
        move(0.01, math.sin(i*math.pi/sp)/sp, 0.0, math.cos(i*math.pi/sp)/sp)
    for i in range(2*sp,0,-1):
        move(0.01, -math.sin(i*math.pi/sp)/sp, 0.0, -math.cos(i*math.pi/sp)/sp)
